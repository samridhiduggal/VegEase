...
const Veggie = ({ handleClick }) => {
  ...
  const filteredvegeitem = vegeitem.filter(itemdata => itemdata.name.toLowerCase().includes(query));
    ...
    <>
      ...
      <div className='card-container'>
        ...
        <div className="card-item" style={{ padding: "2rem" }}>
          {
            filteredvegeitem.length === 0 ? (<h1>No item found</h1>) 
            : (filteredvegeitem.map(item => (
